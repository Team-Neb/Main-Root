...
#pragma comment(lib, "glu32.lib") // pragma is compiler business
...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	{
		MessageBox(NULL,"Failed To Register The Window Class Sir.","ERROR",MB_OK|MB_ICONEXCLAMATION);
								...
								NULL)))								// Dont Pass Anything To WM_CREATE
	{
		...
		MessageBox(NULL,"Errorrrrr.","ERROR",MB_OK|MB_ICONEXCLAMATION);
		return FALSE;								// Return FALSE
	}
		...
		else										// If There Are No Messages
		{
			...
			*/
			...
			if (keys[VK_F1])						// Is F1 Being Pressed?
			{
				...
				if (!CreateGLWindow("Midterm !",fullscreenWidth,fullscreenHeight,256,fullscreen))
				{
					return 0;						// Quit If Window Was Not Created
				}
			}
